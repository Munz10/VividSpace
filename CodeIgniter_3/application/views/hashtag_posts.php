...
<head>
    <meta charset="UTF-8">
    <title>#<?= htmlspecialchars($tag); ?> - VividSpace</title>
...
<div class="container">
    ...
    <h4 class="mt-3">#<?= htmlspecialchars($tag); ?></h4>
    <p class="text-muted"><?= count($posts); ?><?= !empty($has_more) ? '+' : ''; ?> post<?= count($posts) !== 1 ? 's' : ''; ?></p>

    <?php if (empty($posts)): ?>
        <div class="text-center py-5">
            <p class="text-muted" style="font-size:1.1rem;">No posts with this hashtag yet.</p>
        </div>
    <?php else: ?>
        <div class="row mt-3">
    ...
    <div class="modal-dialog modal-lg" role="document">
        <div class="modal-content">
            <div class="modal-header py-2">
                <h6 class="modal-title mb-0">#<?= htmlspecialchars($tag); ?></h6>
...
<script>
    ...
    function toggleLike(postId) {
        ...
        csrfPost('<?= site_url('post/toggle_like'); ?>', { post_id: postId }, function(r) {
            ...
            if (r.status === 'success') { $('#like-count-' + postId).text(r.likes_count); $('#like-btn-' + postId).toggleClass('liked', !!r.is_liked); }
